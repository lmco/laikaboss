...
def getConfig(option):
    """
    Return the option out of configs, or out of default_configs, or return an
    empty string.
    """
    ...
    for fname in iter(job_queue.get, 'STOP'):
        ...
        try:
            ...
            if file_len > 20971520 and not nolimit:
                print(
                    "You're trying to scan a file larger than 20mb.." +
                    " Are you sure?\n" +
...
def main():
    ...
    parser.add_option("-p", "--num-procs",
                      ...
                      dest="num_procs",
                      help=(
                        "Specify the number of processors to use for recursion"
                      )
                     )
    ...
    parser.add_option("-l", "--level",
                      ...
                      dest="return_level",
                      help=(
                          "Return Level: minimal, metadata, full [default:"
                          " metadata]"
                      )
                     )
    parser.add_option("-o", "--out-path",
                      ...
                      dest="save_path",
                      help=(
                          "If Return Level Full has been specified, provide a"
                          " path to save the results to [default: current"
                          " directory]"
                      )
                     )
    ...
    parser.add_option("-m", "--ext-metadata",
                      ...
                      dest="ext_metadata",
                      help=(
                        "Specify external metadata to be passed into the scanner."
                      )
                     )
    ...
    parser.add_option("-R", "--recursive",
                      ...
                      dest="recursive",
                      help=(
                          "Enable recursive directory scanning. If enabled,"
                          " all files in the specified directory will be"
                          " scanned. Results will be output to cloudscan.log"
    ...
    # Parse through the config file and append each section
        ...
        try:
            ...
            if file_len > 20971520 and not options.nolimit:
                print(
                    "You're trying to scan a file larger than 20mb.." +
                    " Are you sure?\n"
                    "Use the --remove-limit flag if you really want to do this."
                )
                sys.exit(1)
        except IOError:
            print("\nERROR: The file does not exist: {}\n".format(args[0]))
