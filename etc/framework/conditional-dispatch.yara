...
* META fields
    ...
    * priority     : This optional field can be used to control the order in which
                     your modules will be executed. Within a single rule, modules
                     will run in the order they are listed. If multiple rules match,
                     modules will be run in order of their rule's priority. The
                     default priority is 9, with 1 being the highest priority.
* External Variables
    * ext_parentModules (str) : a space separated list of the modules that have
                                been run on the parent of the current object
                                (default: NONE)
    * ext_sourceModule (str)  : the module that produced this object or NONE if
                                it's the root object (default: NONE)
