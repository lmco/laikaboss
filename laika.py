        ...
        while True:
            ...
            try:
                ...
                Dispatch(
                    ...
                    result,
                    0,
                    externalVars=myexternalVars,
                    #TODO: where is SCAN_MODULES?
                    extScanModules=SCAN_MODULES
                )
                ...
                # TODO: where is LOG_RESULT?
                if LOG_RESULT:
                    log_result(result)

                # TODO: where is LOG_JSON?
                if LOG_JSON:
                    LOCAL_PATH = LOG_JSON
...
def main():
    ...
    parser.add_option(
        ...
        default=8,
        help=(
            "Specify the number of CPU's to use for a recursive scan." +
            " [default:8]"))
    parser.add_option(
        "-l",
        "--log",
    ...
    parser.add_option(
        ...
        dest="scan_modules",
        help=(
            "Specify individual module(s) to run and their arguments." +
            " If multiple, must be a space-separated list."))
    parser.add_option(
        "--parent",
        action="store", type="string",
        dest="parent", default="",
    ...
    parser.add_option(
        ...
        dest="ext_metadata",
        help=(
            "Define metadata to add to the scan or specify a file" +
    ...
    if options.debug:
        ...
        # remove stdout from handlers so that debug info is only written to the
        # file
