...
def _run_module(sm, scanObject, result, depth, args, earlyQuitTime=0):
    '''
    Description: The Dispatch function uses this private method to run a
    specific module against an object. This method recursively calls the
    Dispatch method for any items returned by a module.
     Arguments:
      ...
      - depth: every time dispatch is called, depth is increased by 1.
               may be used to limit recursion.
      - args: a dictionary containing arguments for a module provided by the
            Dispatcher
    '''
    logging.debug(
    ...
    # Add the current scan module to the list of scan modules run against
    # this object
    ...
    # Perform a recursive scan for each item returned by the module
    # (may be none)
    for moduleObject in moduleResult:
        ...
        Dispatch(
            ...
            result,
            depth,
            externalVars=moduleObject.externalVars
        )

def _conditional_scan(scanObject, externalVars, result, depth):
    '''
    Description: This function performs a second pass scan of an object based
    on the results of a previous scan only. The yara rules look at the flags
    applied to this object and determine any additional scanning that may need
    to be performed based on these flags.
    Arguments:
     - scanObject: the current object being scanned
     - result: collects scan results of all objects being scanned in a
               dictionary
     - depth: every time dispatch is called, depth is increased by 1. may be
    ...
    try:
        ...
        yresults = yara_on_demand(
            config.yaraconditionalrules,
            listToSSV(scanObject.flags),
            externals
        )
        moduleQueue = _get_module_queue(
            yresults,
            result,
            scanObject,
            "Conditional Rules"
...
def _addExtMetadata(scanObject, data):
    '''
    Description: Wrapper function around util function to facilitate adding
    external metadata.
    Arguments:
     - scanObject: the current object being scanned
    ...
    # If the data is a string or list, add it as a single value to the 'data'
    # key
...
def _gather_metadata(buffer, externalVars, result, depth, maxBytes):
    '''
    Description: Helper function to set up a scanObject from various metadata
    sources.
    Arguments:
     ...
     - depth: every time dispatch is called, depth is increased by 1. may be
              used to limit recursion.
     - extMetaData: information provided externally that will be attached as
                    metadata to the scanObject
    '''
    # Set up the object.

    contentType = externalVars.contentType if externalVars.contentType else []
    scanObject = ScanObject(
        parent=externalVars.parent,
    ...
    # Check to see if rootUID has been set, if it hasn't, then assume this is
    # the root
    if not result.rootUID:
        result.rootUID = get_scanObjectUID(scanObject)

    # In order to ensure that all objects have a rootUID, set this after we
    # have set the result.rootUID
...
def _get_module_queue(yresults, result, scanObject, metaLabel):
    '''
    Description: Takes the results from a dispatch yara scan and creates a
    priority queue from them. The function also adds dispatch flags if they
    exist in the rule.
    '''
...
def _process_module_queue(moduleQueue, result, depth, scanObject,
                          earlyQuitTime=0):
    '''
    Description: Takes a priority module queue and runs each module in the
    appropriate order. Each module is tracked for uniqueness to prevent
    redundancy.
    '''
    ...
    while True:
        if MAXDEPTH and depth > MAXDEPTH:
            errorText = (
                "Depth has been exceeded. Only the dispatcher will be" +
                " run on this object."
            )
            logging.debug(errorText)
            log_module_error(
...
def close_modules():
    """
    Description: Module callback API caller to close down (destruct) each
    module safely.
    """
...
def Dispatch(
    ...
    conditional=False ):
    """
    Description: By default, this function uses yara to disposition a buffer
    and determine what scan modules should be run against it. The function may
    be called recursively if a scan module returns additional buffers to scan.
    The function collects all results into the original result object passed in
    by the caller for easy retrieval.
    Arguments: (* denotes OPTIONAL parameters):
     ...
     - depth: every time dispatch is called, depth is increased by 1. may be
              used to limit recursion.
     - *externalVars: variables passed in from the caller or other modules
     - *scanModules: this function may be called with predefined scan modules
                     set (string, space delimited)
     - *conditional: determines whether this function has been called as the
                     result of a conditional scan
     - *externalVars: these variables are passed to yara along with the current
                      object to aid in disposition
    """
    skip_timeout = True
    ...
    # modules are unknown
    # Yara is used to disposition the file and determine which modules should be
    # run against it
    # Using the result of each module, it is determined (using a separate yara
    # scan on the flags) whether or not a conditional scan needs to be run.
    if extScanModules is None:

        # Generate the scan object from the parameters
        scanObject = _gather_metadata(
            buffer,
            externalVars,
            result,
            depth,
        ...
        # check to see if this object has a parent, get the modules run
        # against the parent if it exists
        externals = {
            'ext_parentModules': listToSSV(externalVars.parentModules) or 'NONE',
            'ext_sourceModule': externalVars.sourceModule or 'NONE',
            'ext_contentType': listToSSV(scanObject.contentType) or 'NONE',
        ...
        with _with_conditional(skip_timeout) or \
            timeout(global_scan_timeout, exception=GlobalScanTimeoutError):
            try:
                ...
                # If the scan times out, add a flag and continue as a normal
                # error
                scanObject.addFlag("dispatch:err:scan_timeout")

                # If not the root object, raise the exception to halt the
                # parent scan
                if (depth > 0 and
                    (externalVars is None or depth > int(externalVars.depth))):
                    raise
    ...
    # This branch is designed for externally specified scan modules to be run
    # against a buffer.
    # It is not necessary to disposition the file type with yara because we are
    # trusting the caller.
    # This branch differs from a conditional scan in that there is no metadata
    # about the buffer already,so it must be gathered before beginning the scan.
    # It is also subject to conditional scanning.
    ...
    # This branch is specifically for conditional scans kicked off by this
    # function. Metadata about the object has already been collected and all
    # that needs to occur is scans by the specified modules.
    else:
