...
def _create_word_list(content):
    '''
    Get up to the first 200 words from the content. The content is expected to
    be an email, so all lines before the first blank line are skipped as they
    are assumed to be the header. Any lines after
    'Content-Disposition: attachment' are also skipped as they are assumed to
    be the footer.
    '''
    words = list()
    header = 1
    ...
    def _run(self, scanObject, result, depth, args):
        ...
        with tempfile.NamedTemporaryFile(dir=temp_dir) as temp_file:
            ...
            try:
                ...
                logging.debug('Invalid Rar file')
            ...
            if ((headers_are_encrypted or files_are_encrypted) and
                ...
                for possible_password in possible_passwords:
                    try:
                        ...
                        # Extraction is needed to force the exception on
                        # encrypted files
