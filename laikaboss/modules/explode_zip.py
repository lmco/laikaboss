...
class EXPLODE_ZIP(SI_MODULE):
    ...
    def _unzip_file(
        ...
        If the file is encrypted (determined by an exception), add the flag and
        return
        ...
        scanObject -- an instance of the ScanObject class, created by the
                      dispatcher
        result -- an instance of the ScanResult class, created by the caller
        password -- the password for the zipfile, if any
        file_limit -- the maximum number of files to explode, adds flag if
                      exceeded
        byte_limit -- the maximum size in bytes for an exploded buffer, adds
                      flag if exceeded
            ...
            for name in namelist:
                if byte_limit:
                    ...
                    if info.file_size > byte_limit:
                        logging.debug(
                            "EXPLODE_ZIP: skipping file due to byte limit")
    ...
    def _fix_bad_zip(file, buffer):
        '''
        Python's zipfile module does not tolerate extra data after the central
        directory signature in a zip archive. This function truncates the file
        so that the python zipfile module can properly extract the file.
