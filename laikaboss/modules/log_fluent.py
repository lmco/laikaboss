...
class LOG_FLUENT(SI_MODULE):
    ...
    def _run(self, scanObject, result, depth, args):
        ...
        bufmax = int(get_option(args, "bufmax", "fluent_bufmax", 1048576)) # 1MB
        ...
        for sender in self._senders.itervalues():
            if sender.pendings:
                sender.emit(
                    'flush',
                    {'message':
                        'This event is to flush out the buffer of the' +
    ...
    def _log_record_strainer(self, thing):
        ...
        if thing_type in [list, set, frozenset]:
            ...
            return str(thing)
        # JSON does not fully support NaN and Infinity in its spec, so some
        # JSON libraries do not handle these values and instead raise an error.
        # Serializing these to the string respresentation to prevent these
        # errors from disrupting logging.
