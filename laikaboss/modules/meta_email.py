...
class META_EMAIL(SI_MODULE):
    def __init__(self,):
        ...
        # Domain regex
        self.domainMatch = re.compile(
            r"(\b((?=[a-z0-9-]{1,63}\.)(xn--)?[a-z0-9]+" +
            "(-[a-z0-9]+)*\.)+[a-z]{2,63}\b)")
        # Email Address
        self.emailMatch = re.compile(
            r"(\b[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,}\b)")
        # IPv4 Address
        self.ipMatch = re.compile(
            r'(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)' +
            '\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)')
        # IPv6 Address
        self.ipv6Match = re.compile(
            r'((([0-9A-Fa-f]{1,4}:){7}([0-9A-Fa-f]{1,4}|:))|' +
            '(([0-9A-Fa-f]{1,4}:){6}(:[0-9A-Fa-f]{1,4}|((25[0-5]|' +
            '2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|2[0-4]\d|1\d\d|' +
            '[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){5}(((:[0-9A-Fa-f]{1,4})' +
            '{1,2})|:((25[0-5]|2[0-4]\d|1\d\d|[1-9]?\d)(\.(25[0-5]|' +
            '2[0-4]\d|1\d\d|[1-9]?\d)){3})|:))|(([0-9A-Fa-f]{1,4}:){4}' +
            '(((:[0-9A-Fa-f]{1,4}){1,3})|((:[0-9A-Fa-f]{1,4})?:((25[0-5]|' +
    ...
    def _run(self, scanObject, result, depth, args):
        ...
        for key, value in e.items():
            ...
            #key = key.replace(".", "[d]") # Removing as this will be handled
            # in the framework
            ...
            if key.lower() == "dkim-signature":
                ...
                # add one at a time due to current implementation
                for detail in detailArray:
                    metaDict = self._addToMetaDict(
                        metaDict,
                        "dkim-signature-search",
                        detail
                    )
        ...
        # Copy the headers and run them through decode to pull out the
        ...
        for key, value in metaDictDecode.iteritems():
            try:
                ...
                # if the encoding it something other than utf-8, attempt to
                # convert it
                if format and format != 'utf-8':
                    metaDictDecode[key] = unicode(decoded,
                                                  format).encode('utf-8')
                # format is either empty (assumes ASCII) or utf-8
        ...
        if message_id_domain:
            scanObject.addMetadata(self.module_name,
                                   "MessageID_Domain", message_id_domain)

        scanObject.addMetadata(self.module_name, "String_IPs", sIParray)
        scanObject.addMetadata(self.module_name, "Domains", domainArray,
                               unique=True)
        ...
        scanObject.addMetadata(self.module_name, "Headers-Decode",
                               metaDictDecode)
