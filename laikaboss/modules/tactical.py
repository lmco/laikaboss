...
class TACTICAL(SI_MODULE):
    ...
    def _run(self, scanObject, result, depth, args):
        ...
        # only do something if script is defined in dispatcher--without
        # external script this does nothing
        if 'script' in args:
            ...
            with tempfile.NamedTemporaryFile(dir=self.TEMP_DIR) as temp_file:
                ...
                for line in output.splitlines():
                    ...
                    if line_type == "FLAG:":
                        # not doing any validation on the flags, but truncating
                        # on length
                        ...
                        if os.path.isdir(line_value):
                            # If the file is a directory and we don't already
                            # know about it, add it to the list
                            if line_value not in tmp_dirs:
                                tmp_dirs.append(line_value)
                            # Skip this since it's a directory
                            continue
                        # If we don't already know about this directory,
                        # add it to the list
    ...
    #function to do the whole backtick thing. This is like check_output of
    # python 2.7 without the checking of return value
    @staticmethod
