...
class ScanObject(object):
    ...
    def addMetadata(self, moduleName, key, value, unique=False):
        # Convert the value into UTF8, regardless of type (function will handle
        # it)
        value = convertToUTF8(value)
        key = cleanKey(key)

        # If no metadata exists for this module yet, add a new dictionary with
        # the key/value pair
        if moduleName not in self.moduleMetadata:
            self.moduleMetadata[moduleName] = { key : value }
        # If metadata already exists for this module, first check if the key
        # exists
        else:
            ...
            # Otherwise, check to see if its a list
                ...
                if type(self.moduleMetadata[moduleName][key]) is list:
                    # Check to see if it's in the list. If it is and unique is
                    # specified, don't add it
                    if isinstance(value, list):
                        if unique:
                            self.moduleMetadata[moduleName][key].extend(
                                [x for x in value if x not in
                                 self.moduleMetadata[moduleName][key]])
                        else:
                            self.moduleMetadata[moduleName][key].extend(value)
                    else:
                        if (value not in self.moduleMetadata[moduleName][key] or
                            not unique):
                ...
                # If it's not a list, convert it to one.
                    ...
                    if isinstance(value, list):
                        if unique:
                            metalist.extend(
                                [x for x in list(set(value)) if x !=
                                 self.moduleMetadata[moduleName][key]])
                        else:
    ...
    # If you don't specify a key this function returns a dictionary containing
